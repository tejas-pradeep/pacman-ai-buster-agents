    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for positions in self.legalPositions:
            newPosition = self.getPositionDistribution(self.setGhostPosition(gameState, positions))
            for new, probab in newPosition.items():
                possible[new] += probab * self.beliefs[positions]
        possible.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        k = self.numParticles
        lis = []
        while k > len(self.legalPositions):
            lis += self.legalPositions
            k -= len(self.legalPositions)
        self.particles = lis
    ...
    def observe(self, observation, gameState):
        ...
        j = util.Counter()
        if noisyDistance is None:
            j[self.getJailPosition()] = 1.0
            self.beliefs = j
        else:
            temp = util.Counter()
            for p in self.particles:
                d = util.manhattanDistance(p, pacmanPosition)
                temp[p] += emissionModel[d]
            self.beliefs = temp
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(len(self.particles)):
            ...
            newpdist = self.getPositionDistribution(self.setGhostPosition(gameState, par))
            self.particles[i] = util.sample(newpdist)
...
class JointParticleFilter:
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      "*** YOUR CODE HERE ***"
      for i in range(self.numGhosts):
        dist = getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle),
                                                       i, self.ghostAgents[i])
        newParticle[i] = util.sampleFromCounter(dist)
        "***END CODE HERE***"
      newP.append(tuple(newParticle))
    self.particles = newP
  ...
  def observeState(self, gameState):
    ...
    for p in self.particles:
        for ghosts in ghostss:
            particle = list(p)
            particle[ghosts] = self.getJailPosition(ghosts)
            p = tuple(particle)
        probability = 1
        for i in range(self.numGhosts):
            if i not in ghostss:
