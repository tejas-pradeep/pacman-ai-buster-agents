...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for d in livingGhostPositionDistributions:
            mostLikelyPos = None
            mostLikelyProb = None
            for position, probab in d.items():
                if mostLikelyProb == None or probab > mostLikelyProb:
                    mostLikelyPos = position
                    mostLikelyProb = probab

            for action in legal:
                newPosition = Actions.getSuccessor(pacmanPosition, action)
                new_d = self.distancer.getDistance(newPosition, mostLikelyPos)
                if bestAction == None or new_d < actionDist:
                    bestAction = action
                    actionDist = new_d
        return bestAction
